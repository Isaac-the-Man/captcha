...
os.makedirs('checkpoints/train_crnn', exist_ok=True)
last_chkpt = None
for path in os.listdir('checkpoints/train_crnn'):
    if path.endswith('.pth'):
        epoch = int(path.split('_')[-1].split('.')[0])
        if last_chkpt is None or epoch > last_chkpt:
            last_chkpt = epoch
model = CaptchaModelCRNN().to(device)
...
if last_chkpt is not None:
    checkpoint = torch.load(f'checkpoints/train_crnn/{path}')
...
for epoch in range(last_chkpt, EPOCHS):
    ...
    if epoch % 20 == 0:
        with torch.no_grad():
            ...
            }, f'checkpoints/train_crnn/checkpoint_{epoch}.pth')
